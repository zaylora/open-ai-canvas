...
export function CanvasLeaferGraphicsLayer(props: CanvasLeaferGraphicsLayerProps) {
        ...
        const unsubscribe = subscribeCanvasGraphicsViewportPreview(container, (next) => {
            ...
            if (isViewportPreview(container, next, rasterViewportRef.current)) {
                if (shouldRebaseCanvasRaster(next, rasterViewportRef.current)) {
                    syncViewport(next, containerSize.width, containerSize.height, overlay, propsRef.current);
    ...
    return <div ref={overlayHostRef} data-canvas-leafer-overlay className="pointer-events-none absolute inset-0 z-[var(--z-canvas-overlay)] overflow-hidden" aria-hidden />;
}
...
function connectionArrowPath(point: Position) {
    // 对齐 SVG 常态层的 marker：markerWidth 10 / markerHeight 8 / refX 9，
    // 即底边落在端点后 9、尖端越过端点 1、半宽 4。原先的 9x10 会让箭头在接管时胖一圈。
    const size = 10;
    const half = 4;
    const tipX = point.x + 1;
    return `M ${tipX} ${point.y} L ${tipX - size} ${point.y - half} L ${tipX - size} ${point.y + half} Z`;
}
